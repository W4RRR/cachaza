...
from .report_common import _evidence_value, _table
...
.edge.filtered{display:none}.zoomed-out .edge:not(.origin-path):not(.active){opacity:.12}.zoomed-out .edge-label:not(.active):not(.origin-path){display:none}
...
function bundleUrls(raw){
  const nodes=(raw.nodes||[]).map(node=>({...node})),edges=(raw.edges||[]).map(edge=>({...edge})),buckets=new Map();
  nodes.filter(node=>node.kind==="url").forEach(node=>{let host;try{host=new URL(node.label).host}catch{return}if(!buckets.has(host))buckets.set(host,[]);buckets.get(host).push(node)});
  const parents=new Map();
  buckets.forEach((members,host)=>{if(members.length<2)return;const id=`url-bundle:${host}`,bundle={id,kind:"url_group",label:`${host} · ${members.length} URLs`,members:members.map(node=>node.id),in_scope:members.some(node=>node.in_scope),sources:[...new Set(members.flatMap(node=>node.sources||[]))],evidence_count:members.reduce((sum,node)=>sum+(node.evidence_count||0),0)};members.forEach(node=>{node.url_parent=id;parents.set(node.id,id)});nodes.push(bundle)});
  const aggregated=new Map();edges.forEach(edge=>{const source=parents.get(edge.source)||edge.source,target=parents.get(edge.target)||edge.target;if(source===target||(source===edge.source&&target===edge.target))return;const key=JSON.stringify([source,target,edge.relationship]);if(!aggregated.has(key))aggregated.set(key,{...edge,source,target,bundle_edge:true})});
  nodes.filter(node=>node.url_parent).forEach(node=>edges.push({source:node.url_parent,target:node.id,relationship:"contains URL"}));
  return{nodes,edges:[...edges,...aggregated.values()]}
}
const graph=bundleUrls(report.graph||{nodes:[],edges:[]}),nodes=graph.nodes,byId=new Map(nodes.map(node=>[node.id,node])),edges=graph.edges.filter(edge=>byId.has(edge.source)&&byId.has(edge.target)),layoutNodes=nodes.filter(node=>!node.url_parent),layoutEdges=edges.filter(edge=>!byId.get(edge.source).url_parent&&!byId.get(edge.target).url_parent),expandedUrls=new Map();
...
styles.url_group={color:"#4da3ff",icon:"URLs"};
...
function spatialPairs(points,cellSize,visit){
  const cells=new Map(),key=(x,y)=>`${x},${y}`;
  points.forEach((p,i)=>{const k=key(Math.floor(p.x/cellSize),Math.floor(p.y/cellSize));if(!cells.has(k))cells.set(k,[]);cells.get(k).push(i)});
  points.forEach((a,i)=>{const x=Math.floor(a.x/cellSize),y=Math.floor(a.y/cellSize),near=[];for(let dx=-1;dx<=1;dx++)for(let dy=-1;dy<=1;dy++)for(const j of cells.get(key(x+dx,y+dy))||[])if(j>i)near.push(j);near.sort((a,b)=>a-b).forEach(j=>visit(a,points[j]))})
}
// Exact finite-radius repulsion; distant nodes exert no force in this model.
function settleCollisions(positions,ids,gap){
  const cells=new Map(),key=(x,y)=>`${x},${y}`;
  for(const id of ids){const original=positions.get(id);let point={...original},attempt=0;
    const overlaps=p=>{const x=Math.floor(p.x/gap),y=Math.floor(p.y/gap);for(let dx=-1;dx<=1;dx++)for(let dy=-1;dy<=1;dy++)for(const other of cells.get(key(x+dx,y+dy))||[])if(Math.hypot(p.x-other.x,p.y-other.y)<gap)return true;return false};
    while(overlaps(point)){attempt++;const angle=attempt*2.399963229728653,radius=gap*Math.sqrt(attempt)*.65;point={x:original.x+Math.cos(angle)*radius,y:original.y+Math.sin(angle)*radius}}
    positions.set(id,point);const k=key(Math.floor(point.x/gap),Math.floor(point.y/gap));if(!cells.has(k))cells.set(k,[]);cells.get(k).push(point)
  }
}
function networkLayout(){
  ...
  for(let tick=0;tick<iterations;tick++){
    ...
    spatialPairs(nodes,90,(a,b)=>{const dx=b.x-a.x,dy=b.y-a.y,distance=Math.max(1,Math.hypot(dx,dy)),minimum=72+(a.degree===0||b.degree===0?18:0);if(distance<minimum){const force=(minimum-distance)*.018*cooling,fx=dx/distance*force,fy=dy/distance*force;a.vx-=fx;a.vy-=fy;b.vx+=fx;b.vy+=fy}});
...
function groupedLayout(){
  const positions=new Map(),columns=Math.ceil(Math.sqrt(Math.max(1,present.length)));
  const sizes=present.map(kind=>{const group=[...(groups.get(kind)||[])].filter(node=>!node.url_parent).sort((a,b)=>b.degree-a.degree||a.label.localeCompare(b.label));return {kind,group,radius:Math.max(100,Math.sqrt(group.length)*60+70)}});
  let y=0;
  for(let row=0;row<sizes.length;row+=columns){const items=sizes.slice(row,row+columns),height=Math.max(...items.map(item=>item.radius))*2+100;let x=0;
    items.forEach(({kind,group,radius})=>{const center={x:x+radius,y:y+height/2};groupCenters.set(kind,center);group.forEach((node,index)=>{const angle=index*2.399963229728653,r=index?Math.sqrt(index)*60:0;positions.set(node.id,{x:center.x+Math.cos(angle)*r,y:center.y+Math.sin(angle)*r})});x+=radius*2+100});y+=height
  }
  return positions
}
...
const settledLayouts=new Map();
function layoutPositions(name=currentLayout){
  const cacheKey=JSON.stringify([name,spacingScale,[...expandedUrls]]);
  if(settledLayouts.has(cacheKey))return new Map([...settledLayouts.get(cacheKey)].map(([id,p])=>[id,{...p}]));
  const source=name==="groups"?groupPositions:name==="origin"?originPositions:networkPositions,positions=new Map([...source].map(([id,point])=>[id,scalePosition(point)]));
  const ids=[...positions.keys()].sort((a,b)=>Number(Boolean(byId.get(b).is_origin_path||byId.get(b).is_primary_origin))-Number(Boolean(byId.get(a).is_origin_path||byId.get(a).is_primary_origin)));
  settleCollisions(positions,ids,85*spacingScale);
  nodes.filter(node=>node.members).forEach(parent=>{const center=positions.get(parent.id);parent.members.forEach((id,index)=>{const angle=index*2.399963229728653,radius=(130+Math.sqrt(index)*75)*spacingScale;positions.set(id,{x:center.x+Math.cos(angle)*radius,y:center.y+Math.sin(angle)*radius});if(index<(expandedUrls.get(parent.id)||0))ids.push(id)})});
  settleCollisions(positions,ids,85*spacingScale);
  if(settledLayouts.size>=4)settledLayouts.delete(settledLayouts.keys().next().value);
  settledLayouts.set(cacheKey,new Map([...positions].map(([id,p])=>[id,{...p}])));return positions
}
function updateClusterGuides(){clusterGuides.forEach(({kind,orbit,label})=>{const group=groups.get(kind)||[],points=group.map(node=>groupPositions.get(node.id)).filter(Boolean).map(scalePosition),center=scalePosition(groupCenters.get(kind)||{x:width/2,y:height/2}),radius=Math.max(82,...points.map(point=>Math.hypot(point.x-center.x,point.y-center.y)+38));orbit.setAttribute("cx",center.x);orbit.setAttribute("cy",center.y);orbit.setAttribute("r",radius);label.setAttribute("x",center.x);label.setAttribute("y",center.y-radius-18)})}
...
const visible=node=>activeKinds.has(node.kind)&&(!node.url_parent||(byId.get(node.url_parent).members.indexOf(node.id)<(expandedUrls.get(node.url_parent)||0))),matchesQuery=node=>!query||`${node.label} ${node.kind} ${(node.sources||[]).join(" ")}`.toLowerCase().includes(query);
...
function renderInspector(node){
  ...
  if(node.members){const count=expandedUrls.get(node.id)||0,block=document.createElement("div"),expand=document.createElement("button"),collapse=document.createElement("button"),note=document.createElement("p");expand.className=collapse.className="graph-action";expand.textContent=count?"Show 50 more URLs":"Expand URLs";expand.disabled=count>=node.members.length;collapse.textContent="Collapse URLs";collapse.disabled=!count;note.textContent=`${Math.min(count,node.members.length)} of ${node.members.length} URLs displayed. URLs remain available in the evidence explorer.`;expand.onclick=()=>{expandedUrls.set(node.id,Math.min(node.members.length,count+50));activeKinds.add("url");applySpacing();updateVisibility();renderInspector(node)};collapse.onclick=()=>{expandedUrls.delete(node.id);updateVisibility();renderInspector(node)};block.append(note,expand,collapse);inspector.append(block)}
...
function setZoom(scale,anchor={x:width/2,y:height/2}){const next=Math.max(.05,Math.min(2.6,scale));view.x=anchor.x-(anchor.x-view.x)*next/view.scale;view.y=anchor.y-(anchor.y-view.y)*next/view.scale;view.scale=next;applyView()}
function fitGraph(){
  const visibleNodes=nodes.filter(visible);if(!visibleNodes.length){view={x:0,y:0,scale:1};applyView();return}
  const minX=Math.min(...visibleNodes.map(node=>node.x))-50,maxX=Math.max(...visibleNodes.map(node=>node.x))+225,minY=Math.min(...visibleNodes.map(node=>node.y))-55,maxY=Math.max(...visibleNodes.map(node=>node.y))+55,padding=70,scale=Math.max(.05,Math.min(1.55,Math.min((width-padding*2)/(maxX-minX),(height-padding*2)/(maxY-minY))));
  view.scale=scale;view.x=(width-(minX+maxX)*scale)/2;view.y=(height-(minY+maxY)*scale)/2;applyView()
}
function centerOn(node,scale=1.45){view.scale=Math.max(.05,Math.min(2.6,scale));view.x=width/2-node.x*view.scale;view.y=height/2-node.y*view.scale;applyView()}
...
graphSearch.addEventListener("keydown",event=>{if(event.key!=="Enter")return;const first=nodes.find(node=>matchesQuery(node));if(first){activeKinds.add(first.kind);if(first.url_parent){expandedUrls.set(first.url_parent,byId.get(first.url_parent).members.indexOf(first.id)+1);applySpacing()}updateVisibility();selectNode(first);centerOn(first,Math.max(1.1,view.scale))}});
