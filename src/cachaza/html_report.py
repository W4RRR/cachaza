...
def render_html(data: dict[str, Any]) -> str:
    ...
    origin_hero = (
        ...
        '<div><span class="origin-hero-label">Most likely real origin IP</span>'
        f'<code>{escape(str(origin_ip or "Not identified"))}</code></div>'
...
<details class="section" open id="origin-discovery-section"><summary>Automatic Origin discovery</summary><div class="section-body"><p class="section-note">__ORIGIN_SUMMARY__</p>__ORIGIN_HERO____ORIGIN_RANKING____ORIGIN_TABLE__</div></details>
...
const order=["domain","origin_candidate","url","api_endpoint","email","phone","address","waf","dns_zone_transfer","ip","cidr","asn","organization","network_registration","service","technology","whois","cloud_asset","cloud_provider","fingerprint"],present=[...new Set(nodes.map(node=>node.kind))];
...
const styles={domain:{color:"#3ddc97",icon:"D"},origin_candidate:{color:"#00e5b5",icon:"OR"},url:{color:"#4da3ff",icon:"↗"},api_endpoint:{color:"#38bdf8",icon:"API"},email:{color:"#f0abfc",icon:"@"},phone:{color:"#fbbf24",icon:"T"},address:{color:"#a3e635",icon:"A"},waf:{color:"#fb7185",icon:"WAF"},dns_zone_transfer:{color:"#ef4444",icon:"AX"},ip:{color:"#b779ff",icon:"IP"},cidr:{color:"#ff9f43",icon:"/"},asn:{color:"#ff5d8f",icon:"AS"},organization:{color:"#f7c948",icon:"O"},network_registration:{color:"#35d0ba",icon:"R"},service:{color:"#fb7185",icon:":"},technology:{color:"#22d3ee",icon:"{}"},whois:{color:"#94a3b8",icon:"W"},cloud_asset:{color:"#8ee34d",icon:"☁"},cloud_provider:{color:"#5ee3a1",icon:"C"},fingerprint:{color:"#a78bfa",icon:"#"},other:{color:"#94a3b8",icon:"•"}},styleFor=kind=>styles[kind]||styles.other,kindName=kind=>String(kind||"finding").replaceAll("_"," ");
...
function renderInspector(node){
  ...
  if(node.kind==="origin_candidate"){const facts=document.createElement("dl");facts.className="inspector-facts";[["Origin probability",`${node.origin_probability_percent||0}%`],["Confidence",node.confidence_band||"inconclusive"],["Classification",node.classification||"inconclusive"],["Method",node.probability_method||"heuristic correlation score"]].forEach(([key,value])=>addRow(facts,key,value));inspector.append(facts)}
