...
function groupedLayout(){
  ...
  present.forEach((kind,groupIndex)=>{const group=[...(groups.get(kind)||[])].filter(node=>!node.url_parent).sort((a,b)=>b.degree-a.degree||a.label.localeCompare(b.label)),groupAngle=groupIndex/count*Math.PI*2-Math.PI/2,center={x:width/2+Math.cos(groupAngle)*radiusX,y:height/2+Math.sin(groupAngle)*radiusY};group.forEach((node,index)=>{if(index===0){positions.set(node.id,{...center});return}const angle=index*2.399963229728653+groupAngle,radius=48+Math.sqrt(index)*45;positions.set(node.id,{x:center.x+Math.cos(angle)*radius,y:center.y+Math.sin(angle)*radius})})});
  return positions
}
...
function layoutPositions(name=currentLayout){
  ...
  // Resolve collisions after layout normalization; spacing changes actual node clearance.
  const ids=[...positions.keys()],gap=85*spacingScale;
  for(let pass=0;pass<35;pass++){let moved=false;for(let i=0;i<ids.length;i++)for(let j=i+1;j<ids.length;j++){const a=positions.get(ids[i]),b=positions.get(ids[j]);let dx=b.x-a.x,dy=b.y-a.y,d=Math.hypot(dx,dy);if(d>=gap)continue;if(d<.01){dx=1;dy=0;d=1}const shift=(gap-d)/2;a.x-=dx/d*shift;a.y-=dy/d*shift;b.x+=dx/d*shift;b.y+=dy/d*shift;moved=true}if(!moved)break}
  nodes.filter(node=>node.members).forEach(parent=>{const center=positions.get(parent.id);parent.members.forEach((id,index)=>{const angle=index*2.399963229728653,radius=(130+Math.sqrt(index)*75)*spacingScale;positions.set(id,{x:center.x+Math.cos(angle)*radius,y:center.y+Math.sin(angle)*radius})})});return positions
}
