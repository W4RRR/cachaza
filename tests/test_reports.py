...
class InteractiveReportTests(unittest.TestCase):
    ...
    def test_html_contains_expandable_evidence_and_embedded_graph(self) -> None:
        ...
        self.assertIn('hoveredNode=node;showTooltip(node,event);renderInspector(node)', document)
        ...
        self.assertIn("function originPathLayout(basePositions)", document)
        self.assertIn("originPositions=originPathLayout(networkPositions)", document)
        self.assertIn("(a.stage_number??999)-(b.stage_number??999)", document)
